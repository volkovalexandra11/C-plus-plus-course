...
public:
	...
	unordered_map<string, Node> routeInfo(const string& routeNumber) {
		...
		if (routes_.count(routeNumber)) {
			...
			result["route_length"] =
					Node(static_cast<int>(*routes_.at(routeNumber).lengthByData()));
			result["curvature"] = Node(*routes_.at(routeNumber).curvature());
			result["stop_count"] = Node(static_cast<int>(stops));
			result["unique_stop_count"] = Node(static_cast<int>(uniqueStops));
	...
	unordered_map<string, Node> stopInfo(const string& stopName) {
		unordered_map<string, Node> result;

		if (stops_.count(stopName)) {
			const auto& buses = stops_.at(stopName).buses_;
			vector<Node> busesForStop;
			for (const auto& bus: buses) {
				busesForStop.push_back(Node(bus));
			}

			result["buses"] = Node(busesForStop);
		} else {
			result["error_message"] = Node("not found"s);
		}
...
Stop buildStop(const map<string, Node>& reqMap) {
	Stop stop;
...
BusRoute buildRoute(const map<string, Node>& reqMap) {
...
BusManager buildDataBase(const vector<Node>& requests) {
	Stops stops;
	Routes routes;
