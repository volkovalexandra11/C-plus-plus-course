...
public:
	...
	using Items = array<std::pair<TAirport, int64_t>, static_cast<uint32_t>(TAirport::Last_)>;

	Items GetItems() const {
		Items result;
		for(size_t i = 0; i < stat_.size(); i++) {
			result[i] = make_pair(static_cast<TAirport>(i), stat_[i]);
		}
		return result;
	};

private:
	array<int64_t, static_cast<uint32_t>(TAirport::Last_)> stat_;
};
